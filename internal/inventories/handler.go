...
type InventoriesHandler struct {
	Inventories services.InventoriesService
	Items       services.ItemsService
}

func NewHandler(db *database.Queries) *InventoriesHandler {
	return &InventoriesHandler{
		Inventories: *services.NewInventoriesService(db),
...
func (h *InventoriesHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	uip := mapUpdateParams(inventoryId, accountId, up)
